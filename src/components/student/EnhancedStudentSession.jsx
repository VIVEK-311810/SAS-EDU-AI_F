      ...
      {activePoll ? (
        <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              {activePoll.options && activePoll.options.map((option, index) => (
                <button
                  ...
                  className={`w-full text-left p-4 border-2 rounded-lg transition-all duration-200 ${getOptionColor(index)} ${hasResponded || timeLeft <= 0 ? 'cursor-not-allowed' : 'cursor-pointer'} `}
                >
          ...
          {selectedOption !== null && !hasResponded && timeLeft > 0 && (
            <div className="flex justify-center mt-4">
              <button
                ...
                className="btn-primary px-8 py-3 text-lg bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors duration-200 disabled:bg-blue-300"
              >
          ...
          {hasResponded && submissionResult && (
            <div className={`border rounded-lg p-4 mt-4 ${submissionResult.is_correct
                ? 'bg-green-50 border-green-200'
                : 'bg-red-50 border-red-200'
              }`}>
              <div className="flex items-center">
                <svg className={`h-5 w-5 mr-2 ${submissionResult.is_correct
                    ? 'text-green-600'
                    : 'text-red-600'
                ...
                <span className={`font-medium ${submissionResult.is_correct
                    ? 'text-green-800'
                    : 'text-red-800'
                  }`}>
                  {submissionResult.is_correct
              ...
              {showResults && (
                <>
                  <div className={`mt-3 text-sm ${submissionResult.is_correct
                      ? 'text-green-700'
                      : 'text-red-700'
                    }`}>
          ...
          {hasResponded && !submissionResult && (
            <div className="text-center py-4 text-blue-600">
              <p className="font-medium">Submitting answer...</p>
            </div>
          )}
          ...
          {hasResponded && submissionResult && !showResults && (
            <div className="text-center py-4 text-gray-600">
              <p className="font-medium">Answer locked. Waiting for the teacher to reveal all results...</p>
            </div>
          )}

          {/* Message if time runs out */}
          {timeLeft === 0 && !hasResponded && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center text-red-700 mt-4">
              Time's up! You can no longer respond to this poll.
              {showResults && (
                <>
