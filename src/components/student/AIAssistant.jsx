...
const AIAssistant = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="min-w-0">
              <h1 className="text-xl sm:text-3xl font-bold text-gray-900">AI Assistant</h1>
              <p className="text-gray-600 text-xs sm:text-base truncate">
                {sessionInfo ? `Helping with: ${sessionInfo.session_name}` : 'Your learning companion'}
              </p>
            </div>
          </div>
          <button
            onClick={() => navigate('/student/dashboard')}
            className="text-blue-600 hover:text-blue-800 active:text-blue-900 flex items-center gap-1 sm:gap-2 text-sm sm:text-base py-1 self-start sm:self-auto"
          >
      ...
      {/* Chat Interface */}
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col h-[calc(100vh-180px)] sm:h-[700px]">

        {/* Chat Header */}
        <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white p-3 sm:p-5 rounded-t-xl">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <h3 className="font-semibold text-sm sm:text-lg">AI Study Assistant</h3>
              <p className="text-xs sm:text-sm text-blue-100">Semantic search powered</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-6 bg-gray-50">
          <div className="space-y-3 sm:space-y-4">
            {messages.map((message) => (
              ...
              >
                <div className={`max-w-[90%] sm:max-w-[85%] ${message.type === 'user' ? 'max-w-[85%] sm:max-w-[70%]' : ''}`}>
                  {/* Message Bubble */}
                  <div
                    ...
                    <div className="mt-3 sm:mt-4 space-y-2 sm:space-y-3">
                      {message.searchResults.map((result, index) => (
                        <div key={index} className="bg-white border border-gray-200 rounded-lg p-3 sm:p-4 hover:shadow-md transition-shadow">
                          {/* Result Header */}
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-1 sm:gap-3 mb-2 sm:mb-3">
                            <div className="flex-1 min-w-0">
                              <h4 className="font-semibold text-blue-600 text-sm sm:text-base truncate">
                                {result.resource_title}
                              </h4>
                              <p className="text-xs text-gray-500">
                                Page {result.pageNumber} • {(result.similarityScore * 100).toFixed(0)}% match
                              </p>
                            </div>
                            <span className={`self-start px-2 py-0.5 sm:py-1 text-xs font-medium rounded-full flex-shrink-0 ${getTypeColor(result.resource_type)}`}>
                          ...
                          <a
                            ...
                            className="inline-flex items-center text-blue-600 hover:text-blue-700 active:text-blue-800 font-medium text-xs sm:text-sm"
                          >
                            View
              ...
              <div className="flex justify-start">
                <div className="bg-white border border-gray-200 rounded-lg p-3 sm:p-4">
                  <div className="flex items-center gap-2 text-gray-600">
                    ...
                    <span className="text-xs sm:text-sm">Searching materials...</span>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="p-3 sm:p-4 border-t border-gray-200 bg-white rounded-b-xl">
          <form onSubmit={handleSendMessage} className="flex gap-2 sm:gap-3">
            <input
              ...
              className="flex-1 px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm sm:text-base"
              disabled={isSearching}
            />
            <button
              type="submit"
              disabled={isSearching || !inputMessage.trim()}
              className="px-4 sm:px-6 py-2.5 sm:py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 active:bg-blue-800 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center gap-1 sm:gap-2 transition-colors text-sm sm:text-base flex-shrink-0"
            >
              {isSearching ? (
                <>
